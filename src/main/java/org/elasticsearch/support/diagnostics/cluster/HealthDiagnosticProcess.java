...
/**
 * {@code HealthDiagnosticProcess} performs a request to <code>{baseUrl}/_cluster/health?level=shards</code> and
 * writes to <code>"cluster_health.json"</code>.
 * <p />
 * As implied by the class extension, this does not currently have any version restrictions.
 */
@Singleton
class HealthDiagnosticProcess extends UnversionedHttpRequestDiagnosticProcess {
    /**
     * Create a new {@link HealthDiagnosticProcess}.
     */
    @Inject
    HealthDiagnosticProcess(HttpRequestFactory httpRequestFactory) {
        super("_cluster/health?level=shards&pretty", "cluster_health.json", httpRequestFactory);
    }
}

...
/**
 * {@code PendingTasksDiagnosticProcess} performs a request to <code>{baseUrl}/_cat/pending_tasks?v</code> and writes to
 * <code>"cat_pending_tasks.txt"</code>.
 * <p />
 * The <code>_cat/pending_tasks</code> API was not added until Elasticsearch 1.0.0.
 */
@Singleton
class PendingTasksDiagnosticProcess extends VersionedHttpRequestDiagnosticProcess {
    ...
    PendingTasksDiagnosticProcess(HttpRequestFactory httpRequestFactory) {
        super("_cat/pending_tasks?v", "cat_pending_tasks.txt", httpRequestFactory, Version.VERSION_1_0, null);
    }
}

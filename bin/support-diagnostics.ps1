...
# API calls that only work with 1.0+
    ...
    Write-Host 'Getting _nodes/stats'
    Invoke-WebRequest $esHost'/_nodes/stats?fields=*&pretty&human' -OutFile $outputDir/nodes_stats.json

    Write-Host 'Getting indices stats'
    Invoke-WebRequest $esHost'/_stats?fields=*&pretty&human' -OutFile $outputDir/indices_stats.json
